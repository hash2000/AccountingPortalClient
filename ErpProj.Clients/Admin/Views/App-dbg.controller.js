  ...
  function (
    ...
    return Controller.extend("ErpProj.Home.Views.App", {
      onInit: function () {
        ...
        oRoutesModel.loadData(
          sap.ui.require.toUrl("ErpProj/Home/routes.json"),
          null,
          false
        );
        ...
        me._loginDialogFragment.then(function (oDialog) {
          ...
          $.ajax(ajaxOptions)
            .done(function (result) {
              ...
              me.configureAuthorizationInfo();
              me.addPopoverMessage(
                MessageType.Success,
                "Authorization",
                "Вход в систему",
                "Пользователь '" + oData.username + "' вошёл в систему"
              );
            })
      ...
      configureAuthorizationInfo: function () {
        ...
        if (!auth_data) {
          ...
          oNavigationModel.loadData(
            sap.ui.require.toUrl("ErpProj/Home/navigation-menu.json"),
            null,
            false
          );
